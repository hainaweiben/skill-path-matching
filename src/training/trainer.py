...
class Trainer:
    ...
    def train_epoch(self, train_loader: DataLoader, epoch: int) -> Dict[str, float]:
        ...
        for batch in pbar:
            ...
            targets = batch['label'].to(self.device) if 'label' in batch else None
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader, 
              num_epochs: int, early_stopping: bool = True, patience: int = 5):
        """
        训练模型
    ...
    def test(self, test_loader: DataLoader, metric_functions: Optional[Dict[str, Callable]] = None) -> Dict[str, float]:
        """
        测试模型
    ...
    def _print_progress(self, epoch: int, num_epochs: int, epoch_time: float, train_metrics: Dict[str, float], val_metrics: Dict[str, float]):
        """
        打印训练进度
