...
with open("/home/u2021201733/test/skill_path_matching/data/processed/skill_graph.json") as f:
    data = json.load(f)
...
nx.draw_networkx_edges(G, pos, alpha=0.3, edge_color="gray", width=edge_weights)
...
edge_weights = [G[u][v]["weight"] / 2 for u, v in G.edges()]
nx.draw_networkx_edges(G, pos, alpha=0.2, edge_color="gray", width=edge_weights)
...
for i, community in enumerate(communities):
    ...
    sorted_nodes = sorted(
        [(node, degree_centrality[node]) for node in community_nodes], key=lambda x: x[1], reverse=True
    )
