...
def calculate_ranking_metrics(
    y_true: np.ndarray, y_pred: np.ndarray, k_values: list[int] = [5, 10, 20]
) -> dict[str, float]:
    """
    计算排序指标
    ...
    try:
        ...
        # 当输入数据无效或只有一个类别时会触发
        metrics["auc"] = 0.0
        logging.warning(f"计算 AUC 时出错: {e}")
    ...
    for k in k_values:
        if len(y_true) >= k:
            try:
                ...
                logging.warning(f"计算 NDCG@{k} 时出错: {e}")
...
def calculate_skill_coverage(candidate_skills: list[str], job_skills: list[str]) -> dict[str, float]:
    ...
    if metrics["job_skill_coverage"] + metrics["candidate_skill_utilization"] > 0:
        metrics["skill_f1"] = (
            2
            * metrics["job_skill_coverage"]
            * metrics["candidate_skill_utilization"]
...
def calculate_skill_path_metrics(
    candidate_skill_paths: list[list[str]], job_skill_paths: list[list[str]]
) -> dict[str, float]:
    """
    计算技能路径相关指标
